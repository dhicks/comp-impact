...
```{r fig.width = 1.5 * fig.width, fig.cap = 'Accounts tweeting CSS papers.  \\textbf{A}: Histogram of tweets per account.  \\textbf{B}: Scatterplot of tweets and followers per account.  Colored lines are curves where total reach (no. followers $\\times$ no. tweets) $= k$, for $k = 10^4$ (red), $10^3$ (green), and $10^2$ (blue). \\label{fig.tweeters}'}
...
```{r fig.width = 1.5*fig.width, fig.cap = 'Kernel density estimates of estimated reach. \\textbf{A}: Reach per paper. \\textbf{B}: Reach per month. \\label{fig.reach}'}
...
`r papers %>% filter(lifespan > 0) %>% .[['lifespan']] %>% median(na.rm = T) %>% as.numeric()` days.  See figure \ref{fig.span} and table \ref{tab.span}. 
...
*[SS but weak relationship btwn no. tweets and lifespan]*  Finally, there is no observed relationship between delay and lifespan.  See figure \ref{fig.delay_span}.  
